...
export <- TRUE
if(export){
  pdf("../plots/entropy_surface_failureProbExample_posteriors.pdf")  
}
...
############
PaperBeta.tbl <- data.frame(mean.post = rep(NA, 6), lower.post = NA, 
                             upper.post = NA)
...
rownames(AlphasBeta.tbl) <- c("maximum_entropy", "minimum_KL",
                              ...
                              "hierarchical_LogisticNormal", "Rufo_2012")
...
posterior.densities <- vector(K, mode = "list")
for(k in 1:K){
...
posterior.densities.df <- do.call(rbind, posterior.densities)

expert_priors <- ggplot(posterior.densities.df, aes(x = theta, y = dens,
  ...
  scale_y_continuous(expression(p[i](theta  ~ "|" ~ y)), expand = c(0, 0)) +
  geom_vline(xintercept = y/n, linetype = "longdash", size = 1.5) + 
  theme_bw(base_size = 20)

expert_priors
ggsave(expert_priors, filename = "../plots/posterior_densities_Savchuk.pdf")
...
# Prior
mean(theta.par.dirichlet)
quantile(theta.par.dirichlet, c(.025, .975))

mean(theta.par.logisticNormal)
quantile(theta.par.logisticNormal, c(.025, .975))
...
betadata.stan <-  list(Y = y, X = X, N = n, K = K, a = av - y, b = bv -(n - y))
...
(PaperBeta.tbl[4, 1:3] <- mean_ci(theta.dirichlet) )
...
( PaperBeta.tbl[5, 1:3] <- mean_ci(theta.logisticNormal) )
...
ggsave(plot = radar_alphas, filename = "../plots/alphas_radar_Savchuk_posteriors.pdf")
...
method_posteriors <- ggplot(method.densities.df, aes(x = theta, y = dens,
                                                        linetype = method, colour = method)) + 
  geom_line(size = 2) +
  scale_x_continuous(expression(theta), expand = c(0, 0)) +
  scale_y_continuous(expression(pi(theta  ~ "|" ~ y)), expand = c(0, 0)) +
  geom_vline(xintercept = y/n, linetype = "dashed", size = 1.2) +
  theme_bw(base_size = 16)

method_posteriors
ggsave(method_posteriors, filename = "../plots/method_posterior_densities_Savchuk.pdf")
