...
Z.dirichlet <- mean(apply(alpha.MC.dirichlet, 1, kernel, data = c(y, n)))
Z.logisticNormal <- mean(apply(alpha.MC.logisticNormal, 1, kernel, data = c(y, n)))

(lZ.dirichlet <- log(Z.dirichlet) )
bridgesampling::bridge_sampler(dirichlet.posterior)

(lZ.logisticNormal <- log(Z.logisticNormal) )
...
alpha1 <- rep(1/K, K)
phi(alpha = alpha1, data = c(y,n), lZ = lZ.logisticNormal)
raw <- runif(K); alpha2 <- raw/sum(raw)  
phi(alpha = alpha2, data = c(y,n), lZ = lZ.logisticNormal)
