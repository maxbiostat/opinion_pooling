...
# this set gives interesting results, astar = bstar = 15
a0 <- 20 ; b0 <- 10
a1 <- 10; b1 <- 20
a2 <- 21 ; b2 <- 9
a3 <- 9 ; b3 <- 21
...
export <- TRUE
if(export){
  pdf("../plots/entropy_surface_simuBetaExample.pdf")  
}
...
marginal.likelihoods
normalised.marginal.likelihoods <- marginal.likelihoods/sum(marginal.likelihoods)


MLS <- marginal_likelihood_surface_beta(y = y, n = n, av = av, bv = bv)
export <- TRUE
if(export){
  pdf("../plots/marginalLikelihood_surface_SimulatedBetaExample.pdf")  
}
...
expert_priors
ggsave(expert_priors, filename = "../plots/expert_densities_SimuBeta.pdf")
...
entropy_beta(a = ab.MaxEnt.star[1], b = ab.MaxEnt.star[2])
...
many.kls <- lapply(1:N, function(i) {
  optim(kl.many.startingPoints[i, ], optklbeta_inv, ap = av, bp = bv, type = "pf")
})
...
posterior_experts <- data.frame(
  alpha = as.numeric(c(AlphasBeta.tbl[1, ], AlphasBeta.tbl[2, ], AlphasBeta.tbl[3, ], AlphasBeta.tbl[4, ])),
  lwr = c(rep(NA, 8), post.alpha.cred.dirichlet[1, ], post.alpha.cred.logisticNormal[1, ]),
  upr = c(rep(NA, 8), post.alpha.cred.dirichlet[2, ], post.alpha.cred.logisticNormal[2, ]),
  expert = rep(paste("expert_", 0:(K-1), sep = ""), 4),
  method = rep(c("maximum_entropy", "minimum_KL",  "Dirichlet", "logistic_normal"), each = K)
)
...
ggsave(plot = radar_alphas, filename = "../plots/alphas_radar_SimulatedBeta.pdf")
...
method_posteriors <- ggplot(posterior.densities.df, aes(x = theta, y = dens,
...
method_posteriors
ggsave(method_posteriors, filename = "../plots/method_posterior_densities_SimulatedBeta.pdf")
