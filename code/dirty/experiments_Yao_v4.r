...
writeLines(readLines("stan/optimise_normal_pool_withPrior_unbounded.stan"))

normal.model <- stan_model("stan/optimise_normal_pool_withPrior_unbounded.stan")
...
estimate_once <- function(parms, data){
  ...
  result <- tryCatch(get_estimate(), ## Had to add this because vstar would be Nan sometimes. Might be a bug...
                  error = function(err){
                    ans <- rep(NA, K + 2)
                    names(ans) <-  c("alpha[1]", "alpha[2]", "alpha[3]", "alpha[4]", "alpha[5]", "vstar", "mustar")
                    return(ans)
                  } )
  return(result)
}
...
results.dt <- subset(results.dt, scenario != "scenario_3") ## WARNING: don't forget to remove!
...
all.subsamp.alpha.dt <- subset(all.subsamp.alpha.dt, scenario != "scenario_3") ## WARNING: don't forget to remove!
