...
getPosterior <- function(M, Alpha, cores = 4){
  ...
  P1993_likelihood2 <- compiler::cmpfun(P1993_likelihood2)
  #
  ...
  #
  SpIR.posterior <- logPoolR:::LP_SpIR(
    k = M,
    l = M,
    Model = get_P1993, 
...
Theta.posterior <- data.frame(
  getPosterior(M = 1e5, Alpha = 1/2, cores = 8)$samples
)
...
write.csv(Theta.posterior, 
          file = "saved_data/bowhead_results_SpIR_fixed.csv",
          row.names = FALSE)
