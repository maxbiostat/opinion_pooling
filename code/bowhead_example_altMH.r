...
## Priors
prior <- function(param, a = 1, b = 1){ ## Log-normal priors
  aalphas <- alpha.01(param[3])
  ...
  log(gtools::ddirichlet(aalphas, alpha = c(1/2, 1/2)))
}
## Likelihoods
L2 <- function(x){
...
## Posterior
posterior <- function(param, verbose = TRUE){
  Lk <- Lik(param)
  pr <-  prior(param)
  if(verbose) cat("Likelihood:", Lk, " prior:", pr,
                  " P0:", param[1], " MSYR:", param[2], " alpha:", alpha.01(param[3]),
                  "\n")
  return (Lk + pr)
}
##
...
run_metropolis_MCMC <- function(startvalue, Niter){
  ...
  chain <- Metro_Hastings(li_func = posterior, pars = startvalue,
                          ...
                          # prop_sigma = var(cbind(Theta.posterior[, 1:2],
                          #                        rbeta(nrow(Theta.posterior), 10, 10))),
                          par_names = c('P0', 'MSYR', 'logit_alpha'),
                          quiet = FALSE)
  return(chain)
}
system.time(
  Chain <- run_metropolis_MCMC(
    startvalue = c(mean(P0.prior), mean(MSYR.prior), 0),
    Niter = Niter)
)

MH_LP.Post <- data.frame(Chain$trace[, 1:2],
                         ROI = apply(Chain$trace[, 1:2], 1, getROI),
                         data.frame(t(sapply(Chain$trace[, 3], alpha.01)))
)
names(MH_LP.Post) <- c("P0", "MSYR", "ROI", "alpha_0", "alpha_1")
plot(MH_LP.Post)
...
curve(dgamma(x, shape = 8.2, rate = 372.7),
      ...
      lwd = 2, lty = 2, col = 3, add = TRUE)
#
hist(Chain$trace[, 3], probability = TRUE, main = "",  xlab = expression(alpha))