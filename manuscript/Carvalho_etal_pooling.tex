...
\end{align*}
 ...
 But every non-negative log-concave function $g(x)$ can be represented as $ g(x) = a \cdot c(x) \cdot h(x)$, with $a \geq 0$ and $c(x)$ and $h(x)$ non-negative and log-concave.
 Since under the assumptions on $\boldsymbol F_\theta$ we have that $h(\theta) := \prod_{i=0}^K f_i(\theta)^{w_i}$ is non-negative and log-concave, a contradiction is reached and the result follows. 
\end{proof}
