...
In addition, there is considerable heterogeneity between studies, with (mean) estimates ranging from 6\%~\citep{tun2008sexual} to 24\%\citep{sutmoller2002human,barcellos2003prevalence}.
\begin{table}[ht]
\caption{\textbf{Data extracted from the systematic review and meta analysis conducted by \citet{malta2010hiv} assessing the HIV prevalence among MSM in Brazil.}
...
Study & Reference & $n$ & $y$ &  Beta & Gaussian \\ 
  \hline
0 & \cite{tun2008sexual}            &  658 &  44 & 0.068 (0.050--0.089) & 0.067 (0.048--0.086)\\ 
1 & \cite{barcellos2003prevalence}  &  461 & 111 & 0.242 (0.204--0.282) & 0.241 (0.202--0.280)\\ 
2 & \cite{carneiro2003determinants} &  621 &  61 & 0.100 (0.077--0.124) & 0.098 (0.075--0.122)\\ 
3 & \cite{sutmoller2002human}       & 1165 & 281 & 0.242 (0.218--0.267) & 0.241 (0.217--0.266)\\ 
4 & \cite{BMH2000}                  &  642 &  57 & 0.090 (0.069--0.113) & 0.089 (0.067--0.111)\\ 
5 & \cite{harrison1999incident}     &  849 &  99 & 0.118 (0.097--0.140) & 0.117 (0.095--0.138)\\ 
   \hline
\end{tabular}
\end{table}
...
While the maximum entropy method lead to the same degenerate solution for both distributions, giving all the weight to the study by~\cite{barcellos2003prevalence}, minimising KL divergence lead to the studies by~\cite{tun2008sexual} and~\cite{barcellos2003prevalence} being given non-zero weights.
Interestingly, while the minimum KL method for the Beta distribution representation lead to roughly equal weights, with the study by~\cite{tun2008sexual} being slightly favoured, the solution for the Gaussian representation assigned a much larger weight to the distribution from~\cite{barcellos2003prevalence} (see Discussion).
...
Future research will explore further applications of logarithmic pooling in statistical learning like for instance combining several posterior predictive distributions obtained from cross-validation (see e.g.~\cite{Yao2018}).
...
The authors would like to thank Professor Adrian Raftery, Professor Christian Genest, Felipe Figueiredo and David Poole for helpful suggestions.
...
Since $\forall j$ we have $\mathbb{E}_{0}[X_j]^{\alpha_j} = \left(\int_{\boldsymbol\Theta}f_0(\theta)\frac{f_j(\theta)}{f_0(\theta)}\right)^{\alpha_j}\, d\theta=1^{\alpha_j}$, Theorem~\ref{thm:normalisation} is proven.
\end{proof}

To establish Remark~\ref{rmk:concavity}, we will need the following result from~\cite{genest1984}.
\begin{lemma}
...
\end{lemma}
Now we can state the proof of Remark~\ref{rmk:concavity}.
\begin{proof}
...
With the goal of obtaining a contradiction, suppose that there exists a pooling operator $\mathcal{T}$ that is log-concave but does not enjoy RPC.
 From Lemma~\ref{lem:RPC_representation}, we know that, under this assumption, $\mathcal{T}$ cannot be represented as $\boldsymbol B(\boldsymbol F_\theta) c(\theta) \prod_{i=0}^K f_i(\theta)^{w_i} $.
But every non-negative log-concave function $g(\theta)$ can be represented as
 \begin{equation}
 \label{eq:lc_rep}
  g(\theta) = a \cdot c(\theta) \cdot h(\theta),
 \end{equation}
with $a \geq 0$ and $c(\theta)$ and $h(\theta)$ non-negative and log-concave and  under the assumptions on $\boldsymbol F_\theta$, we have that $h(\theta) := \prod_{i=0}^K f_i(\theta)^{w_i}$ is non-negative and log-concave.
Therefore $\mathcal{T}$ can in fact be represented in the form of~(\ref{eq:lc_rep}) and thus the form of Lemma~\ref{lem:RPC_representation}, contradicting our initial assumption.  
\end{proof}
