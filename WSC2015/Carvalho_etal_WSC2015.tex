...
\section*{Results}
%%%%%%%%%%%%%%%%%%%%%% 
Table \ref{tab:alphasBeta} lists the weights proposed by each method. the first two methods, Maximum Entropy and Minimum KL, suggest to discard some of the  experts in order obtain the ideal pooled distribution according the their respective criteria. Using a Hierarchical Dirichlet prior for the weights 
diverges from the first two methods in proposing almost equal weights. 

More examples an numerical investigation seem to be necessary to decide which pooling strategy is best.
But by looking at the [prior odds/posterior odds] and figure \ref{fig:priors_posteriors_beta}, we can see the all three methods lead 
