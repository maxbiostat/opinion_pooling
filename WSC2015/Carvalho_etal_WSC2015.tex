...
One can get some intuition into the proof  of proposition~\ref{}by noting that maximising~(\ref{eq:entropypiB}) is equivalent to minimizing $\ln t(\boldsymbol\alpha) = -\ln\int_{\boldsymbol\Theta}\prod_{i=0}^{K}f_i(\theta)^{\alpha_i}d\theta$ and therefore maximising  $t(\boldsymbol\alpha)^{-1} = \int_{\boldsymbol\Theta}\prod_{i=0}^{K}f_i(\theta)^{\alpha_i}d\theta$.
All that remains is to show that $t(\boldsymbol\alpha)^{-1}$ has a unique global maximum: %concave or, equivalently, that $t(\boldsymbol\alpha)$ is convex.
[COMPLETE THE PROOF OR OMIT IT]
